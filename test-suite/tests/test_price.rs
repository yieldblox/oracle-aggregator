...
fn test_price() {
    ...
    let xlm_price = oracle_aggregator_client.price(
        &settings_config.assets.get(0).unwrap(),
        &e.ledger().timestamp(),
    );
    match xlm_price {
        Some(price) => {
    ...
    let usdc_config = settings_config.asset_configs.get(1).unwrap();
    let usdc_price = oracle_aggregator_client.price(
        &settings_config.assets.get(1).unwrap(),
        &(e.ledger().timestamp() - usdc_config.resolution),
    );
    match usdc_price {
        Some(price) => {
            assert_eq!(price.price, 0_9900000);
        }
        None => {
            assert!(false)
        }
    ...
    let weth_config = settings_config.asset_configs.get(2).unwrap();
    let weth_price = oracle_aggregator_client.price(
        &settings_config.assets.get(2).unwrap(),
        &(e.ledger().timestamp() - weth_config.resolution * 2),
    );
    match weth_price {
        Some(price) => {
            assert_eq!(price.price, 999_0000000);
        }
        None => {
            assert!(false)
        }
...
fn test_price_circuit_breaker() {
    ...
    let xlm_price = oracle_aggregator_client.price(&asset, &e.ledger().timestamp());
...
fn test_price_asset_not_found() {
    ...
    oracle_aggregator_client.price(
        &Asset::Other(Symbol::new(&e, "NOT_FOUND")),
        &e.ledger().timestamp(),
    );
}
